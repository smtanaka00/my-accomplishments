...
import { Upload, Check, FileType } from 'lucide-react';
import { useGlobalState } from '../context/GlobalStateContext';
import { useNavigate, useLocation } from 'react-router-dom';

const CATEGORIES = [
    'Publication', 'Award', 'Leadership', 'Internal Project',
...
const PATHWAYS = [
    ...
    'Senior Promotion / Performance Review',
    'Graduate School / MBA Application'
];

const EntryForm = () => {
    const { addAchievement, updateAchievement } = useGlobalState();
    ...
    const [impact, setImpact] = useState(editItem?.impact || '');
    const [targetVisa, setTargetVisa] = useState(editItem?.tag || PATHWAYS[0]);
    const [category, setCategory] = useState(editItem?.category || CATEGORIES[0]);
    const [fileName, setFileName] = useState('');
    const [fileData, setFileData] = useState(null);
    const [isPublic, setIsPublic] = useState(editItem?.is_public ?? false);
    ...
    const handleFileChange = (e) => {
        ...
        if (file) { setFileName(file.name); setFileData(file); }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEditMode) {
            await updateAchievement(editItem.id, {
                title, date,
                displayDate: new Date(date + 'T00:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
                category, tag: targetVisa, impact: impact || 'No impact statement provided.', isPublic
            });
    ...
    return (
        <div className="page-container">
            <div className="page-header">
                <h1>{isEditMode ? 'Edit Achievement' : 'Log Achievement'}</h1>
                <p>{isEditMode ? 'Update the details below.' : "Document your win while it's fresh."}</p>
            </div>

            <form onSubmit={handleSubmit}>
                {/* Desktop: two-column grid for cards */}
                <div className="form-grid">
                    ...
                    <div className="flex-col gap-6">
                        ...
                        <div className="card flex-col gap-4">
                            ...
                            <div className="flex-col gap-2">
                                ...
                                <select value={targetVisa} onChange={(e) => setTargetVisa(e.target.value)} style={inputStyle}>
                    ...
                    <div className="flex-col gap-6">
                        ...
                        <div className="card flex-col gap-4">
                            ...
                            <textarea
                                rows="5"
                                placeholder="What was the result? e.g. Reduced lead time by 15%, increasing capacity..."
                                value={impact}
                            ...
                            {!isEditMode && (
                                <div
                                    ...
                                    style={{
                                        ...
                                        backgroundColor: fileName ? 'var(--color-surface)' : 'var(--color-base)',
                                        cursor: 'pointer'
                                    }}
                                >
                                    <input
                                    ...
                                    {fileName ? (
                                        ...
                                        <>
                                            ...
                                            <span className="text-sm font-medium text-muted">Click to Upload Evidence</span>
                        ...
                        {/* Submit */}
                        <button type="submit" className="btn-primary btn-full-width" style={{ padding: 'var(--space-4)', fontSize: '1.05rem' }}>
                            {isEditMode ? 'Save Changes' : 'Save to Vault & Timeline'}
                        </button>
                        ...
                        {isEditMode && (
                            <button
                                ...
                                style={{
                                    ...
                                    borderRadius: 'var(--border-radius-md)', color: 'var(--color-text-muted)',
